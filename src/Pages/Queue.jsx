...
export default function Queue() {
  ...
  const [value, setValue] = React.useState({ questions: '', answer: '', subcategoryID: '' });
    ...
    axios.get("https://interviewback-ucb4.onrender.com/questions/", {
      ...
      .then((res) => {
        ...
        const filterData = res.data.data.filter(Queue =>
          Queue.subcategoryID && Queue.subcategoryID.catagoryID && Queue.subcategoryID.catagoryID.status === 'on'
        );
        setData(filterData);
  ...
  const handleD = (values) => {
    // Check if subcategoryID is valid
      ...
      if (id != null) {
        axios.patch("https://interviewback-ucb4.onrender.com/questions/" + id, values, {
          headers: {
            Authorization: token
          }
        })
                      ...
                      <Box sx={{ minWidth: 120 }}>
                        <FormControl fullWidth>
                          ...
                          >
                            {
                              data2.map((el, i) => (
                                <MenuItem key={el._id} value={el._id}>{el.subCategoryname}</MenuItem>
                              ))
                            }
                          </Select>
                        </FormControl>
            ...
            <tbody>
              ...
              {data.map((el, i) => (
                <tr key={el._id}>
                  ...
                  <td>{el.questions}</td>
                  <td>{el.answer}</td>
                  <td>{el.subcategoryID?.subCategoryname}</td>
                  <td>{el.subcategoryID?.categoryID?.categoryName}</td>
                  <td>
